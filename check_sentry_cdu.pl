#!/usr/bin/perl -w
#/usr/bin/env perl -w - dosnt work on centos, why???
...
#
#############################################################################
#Planned thresholds, but this will be definded in nagios and passed to this script
...
#define the oids
$cduloadoid=".1.3.6.1.4.1.1718.3.2.2.1.7"; #add tower and feed interger. then divide by 100. load means amps.
...
if ($opt_type eq "amp") {
	# Walk to get the loads.
	@loads = `$snmp_walk -v 1 -O vesq -c $opt_com $host $cduloadoid`;
	chomp(@loads);
	
	#check to see if ba is = -1, if so, the second tower is prolly not present, so delete ba-bc
	$ba_load_val = $loads[3];
	if ($ba_load_val == -1) {
		delete $loads[3];
		delete $loads[4];
	...
	$loadSize = @loads;
	$loadSize = scalar (@loads);
	$loadSize = $#loads + 1;
	
	#we need to the divide the load value by 100 so we ge an accurite value.
	$count = 0; 
	while ($count < $loadSize) {
		$current_val = $loads[$count];
		$current_val = $current_val / 100;
		$loads[$count] = $current_val;
	...
	if ($loadSize == 3) {
		$numOfTowers = 1;
		#print "AA:@loads[0], AB:@loads[1], AC:@loads[2]";
        ...
        $numOfTowers = 2;
		#print "AA:@loads[0], AB:@loads[1], AC:@loads[2], BA:@loads[3], BB:@loads[4], BC:@loads[5]";
        
	}
	
	#if the array size does not equal 3 or 6, something is wrong. lets run way, i mean exit...
	else {
		print "Invalid number of infeeds reported by device. A total of 3 or 6 infeeds expected, $loadSize reported.";
	...
	while ($count < $loadSize) {
		...
		elsif ( $current_val ge $warning_thresh) {
			$current_status = 1;
			
		}
        
		#check if value is below warning, then things are ok
		elsif ( $current_val lt $warning_thresh) {
			$current_status = 0;
			
		}
        
		#check if none of the above were met, thow an unknown and exit
		else {
			print "Invalid load value reported by device";
			$current_status = 3;
			exit_sub ();
...
#perhaps check exit status of some of the checks? not sure how to do this yet in perl
#sub check_exit_code () {	
#}
